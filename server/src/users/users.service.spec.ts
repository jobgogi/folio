...
describe('UsersService', () => {
  ...
  describe('deleteUser', () => {
    it('유저를 삭제한다', async () => {
      ...
      // Act & Assert
      await expect(
        service.deleteUser('uuid-2', 'uuid-1'),
    ...
    it('본인 계정은 삭제할 수 없다', async () => {
      ...
      // Act & Assert
      await expect(service.deleteUser('uuid-1', 'uuid-1')).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('ROOT 계정은 삭제할 수 없다', async () => {
      ...
      // Act & Assert
      await expect(
        service.deleteUser('uuid-root', 'uuid-other'),
    ...
    it('존재하지 않는 id면 NotFoundException을 던진다', async () => {
      ...
      // Act & Assert
      await expect(service.deleteUser('not-exist', 'uuid-1')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('uploadAvatar', () => {
    ...
    it('png 파일을 업로드하고 avatar 경로를 반영한다', async () => {
      ...
      mockPrisma.user.update.mockResolvedValue({
        id: 'uuid-1',
        avatar: `${MOCK_NAS_PATH}/.avatars/uuid-1.png`,
      });
      // Act
      const result = await service.uploadAvatar('uuid-1', file);
      // Assert
    ...
    it('허용되지 않는 확장자면 BadRequestException을 던진다', async () => {
      ...
      // Act & Assert
      await expect(service.uploadAvatar('uuid-1', file)).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('getAvatar', () => {
    ...
    it('존재하지 않는 유저면 NotFoundException을 던진다', async () => {
      ...
      mockPrisma.user.findUnique.mockResolvedValue(null);
      // Act & Assert
      await expect(service.getAvatar('not-exist')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('avatar가 없으면 NotFoundException을 던진다', async () => {
      ...
      // Act & Assert
      await expect(service.getAvatar('uuid-1')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
